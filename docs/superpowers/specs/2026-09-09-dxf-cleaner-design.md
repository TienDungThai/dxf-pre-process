...
**Hash không phân biệt chiều vẽ.** Corel thường xuất 2 đường trùng nhau nhưng vẽ ngược chiều nhau (ví dụ khi 1 hình có cả stroke và fill riêng). Nếu hash theo thứ tự (start, end), cặp A→B và B→A sẽ không khớp và bị bỏ sót. Khóa hash phải xây từ tập không thứ tự của 2 đầu mút đã làm tròn (ví dụ `frozenset({start, end})` hoặc tuple đã sắp xếp), không phụ thuộc chiều vẽ. Quy tắc này áp dụng cho cả trường hợp (c) — biên chung giữa hai chi tiết liền kề — vì đó cũng là so khớp segment theo 2 đầu mút.
...
**Dò lại arc trên biên đã weld.** Biên mới sinh ra từ `unary_union` luôn là polyline thô (mất mọi arc/circle gốc), vì Shapely không có khái niệm cung tròn. Với các chi tiết có góc bo tròn hoặc lỗ tròn — rất phổ biến trong thực tế xưởng — việc này làm tăng số node và giảm chất lượng cắt ở đúng những chỗ trước đó là cung tròn sạch. Sau khi có biên welded, chạy lại đúng kỹ thuật circular-fit detection đã dùng ở `flatten.py` (mục 6.2) trên biên mới này: chia biên thành các đoạn liên tiếp, kiểm tra đoạn nào khớp với một cung tròn trong dung sai, rồi thay các đoạn thẳng đó bằng `Segment(arc)`. Bước này chạy trước khi đưa contour welded vào `simplify.py`.
